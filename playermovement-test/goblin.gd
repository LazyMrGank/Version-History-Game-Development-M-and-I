...
@onready var chase_detector = $Visuals/ChaseDetector  # New Area2D for chase range
...
var health: int = 3
var player: Node2D = null  # Reference to the player when in chase range
var chase_speed: float = 48.0  # Slightly faster for chasing, adjust as needed
...
func _ready() -> void:
	...
	chase_detector.body_exited.connect(_on_chase_detector_body_exited)
...
func _physics_process(_delta: float) -> void:
	if is_hit or is_attacking or health <= 0:  # Stop movement if hit, attacking, or dead
		...
		if player:  # Chase player if detected
			move_toward_player()
		else:  # Patrol left and right if no player
			move_character()
			detect_turn_around()
	velocity.y += gravity
	move_and_slide()
...
func move_toward_player() -> void:
	if player and is_on_floor():
		var direction = sign(player.global_position.x - global_position.x)  # 1 if player is to the right, -1 if left
...
func _on_chase_detector_body_entered(body: Node2D) -> void:
	if body.is_in_group("Player") and health > 0:
		player = body  # Set player reference for chasing

func _on_chase_detector_body_exited(body: Node2D) -> void:
	if body.is_in_group("Player"):
		player = null  # Clear player reference to resume patrolling
